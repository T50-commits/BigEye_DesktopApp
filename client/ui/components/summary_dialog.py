...
class SummaryDialog(QDialog):
    def __init__(self, successful: int, failed: int, photo_count: int,
        ...
        self.setWindowTitle("ประมวลผลเสร็จสมบูรณ์")
    ...
    def _setup_ui(self, successful, failed, photo_count, video_count,
        ...
        title = QLabel("\u2705 ประมวลผลเสร็จสมบูรณ์")
        ...
        self._add_row(rl, "รายละเอียด", f"\U0001F4F8 {photo_count} รูป \u00B7 \U0001F3AC {video_count} วิดีโอ", "#8892A8")
        layout.addWidget(results)

        # Credits card
        credits = self._card("เครดิต")
        cl = credits.layout()
        self._add_row(cl, "หักเครดิต", f"{format_number(charged)} เครดิต", "#E8E8E8")
        if refunded > 0:
        ...
        note = QLabel(
            "\U0001F4A1 กรุณาตรวจสอบข้อมูล metadata ก่อนอัปโหลด "
            "ผลลัพธ์จาก AI อาจต้องปรับแต่งเพื่อผลลัพธ์ที่ดีที่สุด"
        )
